          ...
          <b-table show-empty
           ...
           >
            <template slot="first_name" slot-scope="row"> {{row.item.first_name}}</template>
              <template slot="id" slot-scope="row"><a :href="'/crm/clients/show/'+row.item.id">{{row.item.id}}</a></template>
       ...
       // props:items,
        data () {
            return {
                ...
                fields: [
                    ...
                    { key: 'created_at', label: 'Дата регистрации', sortable: true, 'class': 'text-center' },
                    { key: 'actions', label: '' }
                ],
                currentPage: 1,
                perPage: 8,
        ...
        methods: {
            ...
            getItems (ctx = null) {
                ...
                let promise = axios.get('/crm/api/table-clients'+query )
